...
2. **Schema**: [Zod](https://zod.dev/) schema defining the expected output structure
3. **Options**: Configuration including model, history, and other settings

```typescript {4,10} showLineNumbers
...
> [!TIP]
> Providing examples helps guide the AI's output format and improves consistency across multiple runs.
...
```typescript {5,8} showLineNumbers
// Basic streaming
const stream = step(
...
> [!TIP]
> Use `.describe()` to more information about schema fields.

Steps use [Zod](https://zod.dev/) schemas to ensure type safety and guide AI output:
...
**prai** provides specialized step functions for common patterns like processing arrays, filtering data, and combining results - see [Special Step Types](../getting-started/special-step-types.mdx).