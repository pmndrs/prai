...
description: This guide covers advanced **prai** features and patterns for building advanced AI workflows.
nav: 4
---
...
<Keypoints title="What you'll learn">
  <KeypointsItem>**Streaming responses** for real-time feedback</KeypointsItem>
  <KeypointsItem>**Conditional workflows** that branch based on results</KeypointsItem>
  <KeypointsItem>**Model fallback** mechanisms for reliability</KeypointsItem>
  <KeypointsItem>**Testing strategies** with mock providers</KeypointsItem>
</Keypoints>
...
```typescript {3,8} showLineNumbers
...
For testing workflows, use the mock provider. See [Models - Mock Provider](../concepts/model.mdx#mock-provider) for complete documentation.
...
```typescript {1,11,18,26} showLineNumbers
...
```typescript {4,10,16,23} showLineNumbers
...
```typescript {1,11,15} showLineNumbers
