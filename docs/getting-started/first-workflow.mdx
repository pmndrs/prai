...
<Keypoints title="What you'll build">
  <KeypointsItem>A **brand theme generator** workflow</KeypointsItem>
  <KeypointsItem>Multi-step AI processing with **history tracking**</KeypointsItem>
  <KeypointsItem>**Type-safe schemas** with Zod validation</KeypointsItem>
  <KeypointsItem>**CSS output generation** from AI results</KeypointsItem>
</Keypoints>
...
> [!IMPORTANT]
> Make sure you have Node.js installed and an API key from OpenAI, Groq, or Google Gemini before starting.
...
```typescript {1,6,14} showLineNumbers
...
```typescript {4,13} showLineNumbers
...
```typescript {4,9} showLineNumbers
...
2. **Schemas**: How to use [Zod](https://zod.dev/) to enforce output structure
...
- Learn about [Special Step Types](./special-step-types.mdx) for data processing
- Learn about [Advanced Usage](./advanced-usage.mdx) patterns
- Follow [Best Practices](./best-practices.mdx) for production workflows
