...
```typescript {1,14,23} showLineNumbers
...
- **Type-safe**: Use [Zod](https://zod.dev/) schemas to enforce strict output formats
- **Reliable**: Get consistent, predictable results from your AI workflows

## Core Concepts

### Steps
Steps are the fundamental building blocks of prai. Each step represents a single instruction to an AI model with a defined input prompt and expected output schema using [Zod](https://zod.dev/). This ensures type safety and forces the AI to return data in the exact format you need.

Learn more about the [Step Concept](../concepts/step.mdx)
...
Learn more about the [History Concept](../concepts/history.mdx)
...
Learn more about the [Model Concept](../concepts/model.mdx)
...
- [First Workflow](./first-workflow.mdx) - Build your first complete prai workflow
