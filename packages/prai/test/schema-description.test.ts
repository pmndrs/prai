...
import { buildSchemaDescription } from "../src";
import {
  object,
  lazy,
  Schema,
...
describe("schema description", () => {
  ...
  it("should describe object schema with a single field", () => {
    ...
    expect(buildSchemaDescription(schema)).to.equal(
      'an object with the field "name" which is a string'
    );
  });
  ...
  it("should describe union of literals", () => {
    ...
    expect(buildSchemaDescription(schema)).to.equal(
      'a literal "admin", a literal "user", or a literal "guest"'
    );
  });
  ...
  it("should describe record schema", () => {
    ...
    expect(buildSchemaDescription(schema)).to.equal(
      "a record with keys as strings and values as numbers"
    );
  });
